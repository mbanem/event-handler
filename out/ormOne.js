...
import { runCommandStream, log, } from './extension.js';
...
async function createRoleAndDb() {
    ...
    const roleExists = await client.query(`SELECT 1 FROM pg_roles WHERE rolname = $1`, [db.owner]);
    if (roleExists.rowCount === 0) {
        await client.query(`CREATE ROLE "${db.owner}" LOGIN PASSWORD '${db.password}' CREATEDB`);
    }
    log(`Creating database ${db.name}...`);
    // check existence
    const res = await client.query(`SELECT 1 FROM pg_database WHERE datname = $1`, [db.name]);
    if (res.rowCount === 0) {
...
export async function installPrismaPartOne(db_, thePaths) {
    db = db_;
    paths = thePaths;
    log([
        ...
        if (envContent.includes('DATABASE_URL')) {
            if (!envContent.includes(dblink)) {
                ...
                envContent = envContent
                    .trim()
                    .replace(/(DATABASE_URL=)(.*)$/gm, '//\tThe previous connection string\n//\t$1$2\n//\thas been replaced with the following\n' +
                    dblink);
    ...
    log(`forming dblink with db params ${JSON.stringify(db)} ${db.owner} ${db.password} ${db.port} ${db.name}`);
...
//# sourceMappingURL=ormOne.js.map