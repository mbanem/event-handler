...
import { log, error } from './extension.js';
 ...
 */
export function displayWebview(context, panel, pageName, owner) {
 ...
 */
function getWebviewHtml(context, webview, pageName) {
    const nonce = getNonce();
    // Locate the HTML file
    const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
    const extensionRoot = context.extensionUri.fsPath;
    const possibleRoots = [workspaceRoot, extensionRoot].filter(Boolean);
    let htmlPath = '';
    for (const root of possibleRoots) {
        const c1 = path.join(root, 'out', 'webview-assets', `${pageName}.html`);
        const c2 = path.join(root, 'out', 'webview-assets', 'src', 'webview-ui', `${pageName}.html`);
    ...
    const assetsFolder = vscode.Uri.joinPath(context.extensionUri, 'out', 'webview-assets');
    html = html.replace(/(src|href)=["'](\.\/)?([^"']+)["']/gi, (fullMatch, attr, dotSlash, relativePath) => {
        ...
        try {
            const assetUri = webview.asWebviewUri(vscode.Uri.joinPath(assetsFolder, relativePath));
    ...
    html = html.replace(/<\/head>/i, `<meta http-equiv="Content-Security-Policy" content="${csp}">\n</head>`);
    log(`final ${pageName}html length: ${html.length}`);
    return html;
}
function getDevHtml(webview, pageName) {
