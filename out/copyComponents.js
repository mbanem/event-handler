...
context, root, selectedComponents) {
    ...
    const sourceUri = vscode.Uri.joinPath(context.extensionUri, 'templates', 'components');
    const copied = [];
    const failed = [];
    ...
    if (copied.length > 0) {
        vscode.window.showInformationMessage(`✅ Copied ${copied.join(',')} component(s) to src/lib/components`);
    }
    if (failed.length > 0) {
        vscode.window.showWarningMessage(`⚠️ Failed to copy: ${failed.join(', ')}`);
    }
}
//# sourceMappingURL=copyComponents.js.map